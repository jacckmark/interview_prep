...
Function.prototype.myBind = function (thisArg, ...boundArgs) {
  ...
  return function (...args) {
    orgObj.apply(thisArg, [...boundArgs, ...args]);
  };
};
...
const replaceWithCamelCase = txt => {
  const txtArr = Array.isArray(txt) ? txt : txt.split("_");

  if (txtArr.length < 2) return txt;
...
const $ = selector => {
  ...
  return {
    css: function (prop, val) {
      ...
      if (selectedEl != null) {
        return (selectedEl.style[prop] = value);
      }
      return this;
    }
  };
...
const throttle = (callback, timer = 100) => {
  let canBeCalled = true;
