...
const once = callback => {
  ...
  return function (...args) {
    ...
    return (res = callback.apply(context, args));
  };
};
  ...
  return {
    css: function (prop, val) {
      if (val === undefined) {
        ...
        let value = selectedEl.style[prop];
...
const getNestedProp = (obj, path, defaultVal) => {
  ...
  if (!pathArr.length) return defaultVal || undefined;
  let temp = obj;
  ...
  for (let i = 0; i < pathArr.length; i++) {
    ...
    temp = temp[current];
    if (!temp) {
      if (i === pathArr.length - 1) return temp !== undefined ? temp : defaultVal;
